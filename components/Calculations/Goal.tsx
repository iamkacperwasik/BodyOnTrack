...
const Goal = () => {
  ...
  return (
    <div>
      <p className="mb-4 text-2xl sm:text-3xl">Goal</p>

      <select
        value={goal}
        onChange={({target}) => setGoal(target.value as Goal)}
        className="py-1 px-4 sm:text-xl"
      >
      ...
      <Switch>
        <Case condition={goal === "LOSE_WEIGHT"}>
          <span className="sm:text-2xl"> with </span>
          <input
            type="number"
            className="w-min border-[1px] sm:text-xl"
            min={0}
            max={50}
            onChange={({target}) => setPercent(Number(target.value))}
            value={percent}
          />
          <span className="sm:text-2xl">% deficit</span>
        </Case>
        ...
        <Case condition={goal === "GAIN_WEIGHT"}>
          ...
          <input
            ...
            value={percent}
          />
          <span className="sm:text-2xl">% surplus</span>
        </Case>
      </Switch>
    </div>
  )
